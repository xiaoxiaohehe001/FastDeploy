    ...
    def _update_task_results(self, tasks):
        ...
        for i in range(len(tasks)):
            ...
            if self.config.is_arch("chatglm"):
                ...
                tasks[i].decode_status["tgt_pos"] = info["tgt_pos"][i]
            else:
    ...
    def _add_dynamic_batching_inputs(self, inputs, tasks, stop_nums):
        ...
        for i in range(len(tasks)):
            ...
            if tasks[i].status == TaskStatus.DECODING:
                # tgt_pos.append(tasks[i].decode_status["tgt_pos"].flatten().tolist())
                x = tasks[i].decode_status["tgt_pos"].flatten().tolist()
                x[0] = tasks[i].decode_status["seq_lens_decoder"] - tasks[
                    i].decode_status["step_idx"] + 1
                x[1] = tasks[i].decode_status["step_idx"] + 1
                tgt_pos += x
                # logger.info("tgt_pos {} ".format(tgt_pos))
